...
```{r load-libraries, message = FALSE, results = FALSE}
packages <- c(
  "here",
  "tidyverse",
  "cowplot",
...
```{r read-csvs, message = FALSE, include = FALSE}
...
Let's explore the token frequencies for the words (N = `r nrow(token)`. Remember, this just takes a measure of how often things changes across all occurrences of words. This means that words that are very common get counted in the data a lot.
...
```{r make-token-contrastive-donut-data}
...
Using this summary, we can just multiply the proportion data by 100 to get percentages of changes. With this method, we see that across all occurrences of words (token frequency), contrastive words (i.e. those with a lexical/phonological variant) occured `r token_contrastive$proportion*100`% of the time.
...
```{r shifts-by-type, include = FALSE}
token_shift_only_types <- donut_data %>%
  filter(shift_category %in% c("lexical", "phonological")) %>%
  mutate(
    prop = n/sum(n),
    total = sum(n)
...
phon_shifts <- token_shift_only_types %>% 
  filter(shift_category == "phonological")
```

Using the data from the donut plot, it's clear that a phonological change occurs `r donut_data %>% filter(shift_category == "phonological") %>% pull("proportion")*100`% of the time across all books. But, out of all changes, a change is phonological (rather than lexical) `r phon_shifts %>% pull("prop")*100`% of the time.
...
Here we can see that phonogical changes affect function words (`r token_phon_descriptives %>% filter(type == "function") %>% pull(prop)*100`%) and nouns (`r token_phon_descriptives %>% filter(type == "noun") %>% pull(prop)*100`%) of the time. Both of these parts of speech thus involve changes more often than any other part of speech.
...
Using these summaries (which are just numbers, so I won't provide a table), we found that approximately `r with(token_shift_summary, c_drops/(c_drops + subs))`% of all phonological changes involve dropping a consonant from the word, while `r with(token_shift_summary, subs/(c_drops + subs))`% involve substution of a vowel.
...
We'll conduct a brief exploration into the type frequencies of words. This takes all unique words from the books in English (N = `r nrow(type)`), and looks at how often they change, and how they change. This just involves some simple counting. 
...
Here you can see that we have each unique word in Standard British English, followed by a count of how often this word occurs, and many columns indicating counts for the number of overall changes to the word (total_diff), how often no change occurs, and finally how often lexical, phonological, or unsure changes occur (essentially splitting up the total_diff column).
...
Phonological changes occur `r with(type_shifts, phon/n)*100`% of the time across all books, while lexical changes occur `r with(type_shifts, lex/n)*100`% of the time. When a change occurs, this change is phonological (rather than lexical) `r with(type_shifts, phon/(phon + lex))*100`% of the time.
...
```{r changing-types-wordcloud-phon}
ggplot(
...
```{r changing-types-wordcloud-lex}
ggplot(
...
```{r make-summary-for-papers, include = FALSE}
gruff_summary <- tibble(
  label = c(
    "token_contrastive",
    "token_phon",
    "token_constrastive_phon",
    "token_phoneme_subs",
    "token_consonant_drops",
    "token_phon_contrastive_noun",
    "type_contrastive",
    "type_phon",
    "type_constrastive_phon"
  ),
  frequency_proportion = c(
    token_contrastive$proportion, 
    donut_data %>% 
      filter(shift_category == "phonological") %>% 
      pull(proportion),
    token_shift_only_types %>% 
      filter(shift_category == "phonological") %>% 
      pull(prop),
    token_shift_summary$subs/sum(token_shift_type$n),
    token_shift_summary$c_drops/sum(token_shift_type$n),
    token_phon_descriptives %>% filter(type == "noun") %>% pull(prop),
    with(type_shifts, (phon + lex)/n),
    with(type_shifts, phon/n),
    with(type_shifts, phon/(phon + lex))
  )
)
...
write_csv(gruff_summary, here("data", "03_summary-data", "frequencies.csv"))
write_csv(counts, here("data", "03_summary-data", "counts.csv"))
```