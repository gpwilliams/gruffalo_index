...
We can see that across all occurrences of words (token frequency), contrastive words (i.e. those with a lexical/phonological variant) occured `r token_contrastive$proportion*100`% of the time.

```{r shifts-by-type, include = FALSE}
token_shift_types <- token %>%
  group_by(shift_category) %>%
  summarise(
    n = n()
  ) %>%
...
token_shift_only_types <- token_shift_types %>%
  filter(shift_category %in% c("lexical", "phonological")) %>%
  mutate(
    prop = n/sum(n),
    total = sum(n)
...
Phonological shifts occur `r token_shift_types %>% filter(shift_category == "phonological") %>% pull("prop")*100`% of the time across all books. When a shift occurs, this shift is phonological (rather than lexical) `r token_shift_only_types %>% filter(shift_category == "phonological") %>% pull("prop")*100`% of the time.
...
Here we can see that phonogical shifts affect function words (`r token_phon_descriptives %>% filter(type == "function") %>% pull(prop)*100`%) and nouns (`r token_phon_descriptives %>% filter(type == "noun") %>% pull(prop)*100`%) more than any other word type.
...
Approximately `r with(token_shift_summary, c_drops/(c_drops + subs))`% of all phonological shifts involve dropping a consonant from the word, while `r with(token_shift_summary, subs/(c_drops + subs))`% involve substution of a vowel.
...
Phonological shifts occur `r with(type_shifts, phon/n)*100`% of the time across all books. When a shift occurs, this shift is phonological (rather than lexical) `r with(type_shifts, phon/(phon + lex))*100`% of the time.
